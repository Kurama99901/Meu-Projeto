document.addEventListener("DOMContentLoaded", function () {
    ...
    const modalElement = document.getElementById("modalLancamento"); // Referência do modal
    const btnFazerLancamento = document.getElementById("alterarEstoque"); // Botão de confirmar alteração
    const btnFecharModal = document.querySelector(".fechar"); // Botão de fechar modal
    const operacaoSelect = document.getElementById("operacao"); // Seleção do tipo de operação
    const novoProdutoDiv = document.getElementById("novoProduto"); // Div de novo produto
    ...
    const localInput = document.getElementById("local"); // Localização atualize as instâncias para que funcione
...
async function loadEstoqueData() {
    try {
        const response = await fetch(`${apiUrl}/estoque.db`);
...
async function registerMovimentacao(movimentacao) {
   ...
   function saveEstoqueData() {
    ...
    function displaySuggestions(filteredItems) {
      ...
      // Função para atualizar os campos do modal
      function updateModalFields() {
        ...
        // Mostrar ou esconder campos dependendo da operação
        if (operacaoSelect.value === 'novo') {
            novoProdutoDiv.style.display = 'block'; // Exibe a div para novo produto
            responsavelDiv.style.display = 'none'; // Esconde o campo de responsável
        } else if (operacaoSelect.value === 'exclusao') {
            novoProdutoDiv.style.display = 'none';
            responsavelDiv.style.display = 'none';
        } else {
            novoProdutoDiv.style.display = 'none'; // Esconde a div de novo produto
